...
assert.match(atomicSource, /removeSkillInstallReceipt/);
assert.match(atomicSource, /clearOnboardingClientEvidence/);
assert.match(atomicSource, /plan\.clientId === current\.clientProfileId/);
...
assert.match(globalSource, /settings\.skillInstallReceipts = \{\}/);
assert.match(globalSource, /clearOnboardingClientEvidence/);
assert.match(globalSource, /skillPlanActions\.clear\(\)/);
