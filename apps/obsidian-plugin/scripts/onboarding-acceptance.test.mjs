...
try {
	...
	assert.ok(capabilitiesModalSource.includes('人工审核必须在 Obsidian 中完成'));
	assert.ok(capabilitiesModalSource.includes('Human review must happen in Obsidian'));
	assert.ok(capabilitiesModalSource.includes('public MCP tool after a dry-run'));
...
assert.ok(skillPromptSource.includes("case 'location_required'"));
	...
	assert.ok(pluginBundleSource.includes(embeddedBundle.TRACEKEEPER_SKILL_BUNDLE.manifest.skill_version));
