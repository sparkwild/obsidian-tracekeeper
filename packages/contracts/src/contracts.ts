...
export const toolContracts = [
	...
	},
	{
		...
		useCase: 'start_task',
		description:
			'[low-risk write] Begin live task tracking when cross-session continuity, interruption recovery, in-progress visibility, explicit live tracking, or task-linked intermediate writes require a real task identity. Ordinary tracked work can use closeout-only finish_task instead.',
		inputSchema: withToolInput(
			{
	...
	},
	{
		...
		description:
			'[read-only] Inspect pending local Vault proposals, trusted Wiki task batches, or approved writeback candidates. Does not approve or apply changes.',
		inputSchema: withToolInput({
			action: {
				type: 'string',
	...
	},
	{
		...
		description:
			'[review-gated apply] Use only after the user approves a Knowledge Change Review proposal. Appends approved content, updates a bounded managed Wiki relation region, or creates the explicitly approved missing Wiki/MemoryRecord target after a fresh confirmation preview.',
		inputSchema: withToolInput({
	...
	},
	{
		...
		description:
			'[low-risk write] Save user-provided source evidence under Sources. Does not fetch external content and does not apply a Wiki or Memory proposal; a captured Source remains readable provenance, not proof that synthesized knowledge was persisted.',
		inputSchema: withToolInput(
			{
