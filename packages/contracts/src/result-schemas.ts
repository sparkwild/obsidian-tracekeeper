...
export const START_TASK_SUCCESS_OUTPUT_SCHEMA: JsonSchema2020 = {
	...
	properties: {
		...
		tracking_mode: { const: 'live', type: 'string' },
		task_record_origin: { const: 'start_task', type: 'string' },
		started_at: { type: 'string', minLength: 1 },
		started_at_source: { type: 'string', enum: ['server_observed', 'client_claim'] },
		tracking_started_at: { type: 'string', minLength: 1 },
		recorded_at: { type: 'string', minLength: 1 },
		start_recovery: { const: 'not_requested', type: 'string' },
...
export const FINISH_TASK_SUCCESS_OUTPUT_SCHEMA: JsonSchema2020 = {
	...
	properties: {
		...
		activity_path: { type: 'string', minLength: 1 },
		tracking_mode: { type: 'string', enum: ['live', 'closeout_only'] },
		task_record_origin: {
			type: 'string',
			enum: ['start_task', 'finish_task_closeout_only', 'finish_task_reconstruction'],
		},
		started_at: { oneOf: [{ type: 'string', minLength: 1 }, { type: 'null' }] },
		started_at_source: { type: 'string', enum: ['server_observed', 'client_claim', 'unknown'] },
		tracking_started_at: { oneOf: [{ type: 'string', minLength: 1 }, { type: 'null' }] },
		recorded_at: { type: 'string', minLength: 1 },
		start_recovery: { type: 'string', enum: ['not_requested', 'matched', 'not_found'] },
...
export const REQUEST_MAINTENANCE_OUTPUT_SCHEMA: JsonSchema2020 = {
	type: 'object',
	oneOf: [
		{
			type: 'object',
...
const REVIEW_PENDING_ENTRY_SCHEMA = {
	...
	properties: {
		...
		review_batch_id: NULLABLE_STRING_SCHEMA,
		effective_risk: NULLABLE_STRING_SCHEMA,
		effective_wiki_rule: NULLABLE_STRING_SCHEMA,
		wiki_role: NULLABLE_STRING_SCHEMA,
		parent_wiki: NULLABLE_STRING_SCHEMA,
		record_identity: MEMORY_RECORD_IDENTITY_SCHEMA,
	...
	oneOf: [
		{
			...
			properties: {
				...
				batches: { type: 'array', items: OPEN_OBJECT_SCHEMA },
			},
			additionalProperties: false,
		},
		{
	...
	oneOf: [
		{
			type: 'object',
				required: [
					...
					'proposal_id', 'proposal_path', 'target_note', 'target_content_hash', 'touched_notes', 'writeback_effect', 'writeback_preview',
			...
			properties: {
				...
				writeback_effect: {
					type: 'string',
					enum: ['append', 'create_wiki_note', 'create_memory_record', 'update_managed_relations'],
				},
	...
	oneOf: [
		...
		},
		{
			...
			required: [
				'schema_version', 'ok', 'tool', 'operation_id', 'idempotency_key', 'status',
				'persisted', 'warnings', 'auto_applied', 'duplicate', 'proposal_destination',
				'memory_rule', 'memory_scope', 'project_hint', 'target_note', 'proposal_id',
				'proposal_path', 'review_reason', 'review_warnings', 'effective_wiki_rule',
				'next_actions',
			],
			properties: {
				...PROPOSE_MEMORY_COMMON_PROPERTIES,
				...
				persisted: { const: false, type: 'boolean' },
				auto_applied: { const: false, type: 'boolean' },
				proposal_destination: { const: 'wiki', type: 'string' },
				memory_rule: { const: null, type: 'null' },
				memory_scope: { const: null, type: 'null' },
				effective_wiki_rule: { const: 'disabled', type: 'string' },
				proposal_id: { const: null, type: 'null' },
				proposal_path: { const: null, type: 'null' },
			},
			additionalProperties: false,
