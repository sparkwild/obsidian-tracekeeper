...
function boundedReadViewMatches(view, entries, query, scope, nowMs) {
    ...
    const candidatePriority = (entry, rawScore) => {
        ...
        const workRecord = isGeneratedWorkRecord({ relativePath: entry.path });
        const priority = rawScore + (exact ? 8 : 0) + titleHits * 2
            + (entry.path.startsWith(`${core_1.KNOWLEDGE_PROJECTS_MEMORY_DIR}/`) ? PROJECT_MEMORY_RECALL_BOOST : 0)
            + ((0, core_1.isKnowledgeWikiPath)(entry.path) ? KNOWLEDGE_WIKI_RECALL_BOOST : 0)
    ...
    const lexical = [...matchedByPath.entries()]
        .sort(([leftPath, left], [rightPath, right]) => candidatePriority(allowed.get(rightPath), right.size) - candidatePriority(allowed.get(leftPath), left.size)
