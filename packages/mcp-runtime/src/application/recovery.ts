...
export function recoveryRequestForRecord(
	...
	if (record.operation_id.startsWith('source-request-') && payload.kind === 'source-request-v2' && payload.requestSnapshot && typeof payload.requestSnapshot === 'object' && (payload.tool === 'tracekeeper.source_request' || payload.tool === 'tracekeeper.analyze_source_request')) return { tool: payload.tool, args: payload.requestSnapshot as Record<string, unknown> };
...
export class RuntimeRecoveryController {
    ...
    (report.attention ??= []).push({ operation_id: record.operation_id, reason: record.operation_id.startsWith('task-relations-') ? 'Resume the approved migration in Obsidian Task relation maintenance using its verified backup.' : record.operation_id.startsWith('wiki-review-batch-')
