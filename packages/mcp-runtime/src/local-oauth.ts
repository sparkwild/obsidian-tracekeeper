	...
	private handleAuthorizationPage(
		...
		if (parsed instanceof Error) {
			this.writeOAuthHtml(
				response,
				400,
				errorPage('授权请求无效', parsed.message),
				request
			);
			return;
		}
		...
		if (!client || client.expiresAtMs <= Date.now()) {
			this.writeOAuthHtml(
				response,
				400,
				errorPage('客户端注册无效', '请返回 Agent 重新发起连接。'),
				request
			);
			return;
		}
	...
	private async handleAuthorizationPost(
		...
		if (!ticket) {
			this.writeOAuthHtml(
				response,
				400,
				errorPage('配对码无效', '请回到 Obsidian 重新核对，或生成新的配对码。'),
				request
			);
			return;
		}
	...
	private completeAuthorization(
		...
		if (!approval || approval.expiresAtMs <= Date.now()) {
			this.writeOAuthHtml(
				response,
				400,
				errorPage('确认已失效', '请返回 Agent 重新发起连接。'),
				request
			);
			return;
		}
		...
		if (
			...
			this.writeOAuthHtml(
				response,
				400,
				errorPage('配对已失效', '请在 Obsidian 中生成新的配对码。'),
				request
			);
			return;
		}
...
function authorizationRequestInputs(request: AuthorizationRequest): string {
