...
interface RequestMaintenanceArgs extends ToolArgs {
...
interface FinishTaskArgs extends ProjectScopeArgs {
	...
	goal?: unknown;
	started_at?: unknown;
	recording_reason?: unknown;
	start_idempotency_key?: unknown;
	summary?: unknown;
	status?: unknown;
	outcomes?: unknown;
	decisions?: unknown;
...
type DurableProposalStatus = Exclude<DurableOutputStatus, 'none' | 'mixed'>;

interface FinishTaskDurableProposalSnapshot {
	proposalId: string;
	path: string;
	proposalKind: string;
	targetPath: string;
...
interface FinishTaskDurableOutputSnapshot {
	sourceCapturePaths: string[];
	proposals: FinishTaskDurableProposalSnapshot[];
	autoWrites?: Array<{ operationId: string; path: string; status: 'applied' | 'unresolved' }>;
}

interface FinishTaskDurableOutputSummary {
...
interface FinishTaskDurableOutputEvidence {
	summary: FinishTaskDurableOutputSummary;
	proposalIdsAtFinish: string[];
	unresolvedAutoWriteCount?: number;
}
...
function isDurableOutputStatus(value: unknown): value is DurableOutputStatus {
	return value === 'none'
...
function legacyDurableOutputSummary(payload: Record<string, unknown>): FinishTaskDurableOutputSummary {
	const memoryStatus = canonicalMemoryCloseoutStatus(payload);
	const proposals = Array.isArray(payload.proposals)
		? payload.proposals.filter(isRecord)
		: [];
	const autoApplied = Array.isArray(payload.auto_applied_memory_updates)
		? payload.auto_applied_memory_updates.filter(isRecord)
		: [];
	const proposalPaths = normalizedStringList(
		proposals.map((proposal) => proposal.path ?? proposal.proposal_path)
	);
	...
	const activeStates = [pendingReviewCount, appliedCount, unresolvedCount]
		...
		.length;
	const status: DurableOutputStatus = activeStates > 1
		? 'mixed'
...
function durableOutputSummaryFromPayload(payload: Record<string, unknown>): FinishTaskDurableOutputSummary {
	if (!isRecord(payload.durable_output)) {
...
function compatibleMemoryCloseoutStatus(
	durableOutput: FinishTaskDurableOutputSummary,
	fallback: MemoryCloseoutStatus
	...
	if (
		durableOutput.pending_review_count > 0
...
function durableOutputNeedsReview(durableOutput: FinishTaskDurableOutputSummary): boolean {
...
function decorateToolResult(
	...
	if (toolName === 'tracekeeper.finish_task') {
		const durableOutput = durableOutputSummaryFromPayload(payload);
		const memoryStatus = compatibleMemoryCloseoutStatus(
			durableOutput,
			canonicalMemoryCloseoutStatus(payload)
		);
...
async function stableKnowledgeReadViewForContext(
	vaultRoot: string,
	context: ToolInvocationContext
): Promise<KnowledgeReadView> {
...
function coerceIsoTimestamp(value: unknown, required: boolean, field: string): string {
	...
	if (Number.isNaN(parsed)) {
		throw new ToolInputError(`${field} must be a valid ISO timestamp.`);
	}
	return new Date(parsed).toISOString();
}

type TaskTrackingMode = 'live' | 'closeout_only';
type TaskRecordOrigin = 'start_task' | 'finish_task_closeout_only' | 'finish_task_reconstruction';
type StartedAtSource = 'server_observed' | 'client_claim' | 'unknown';
type StartRecovery = 'not_requested' | 'matched' | 'not_found';
type RecordingReason = 'ordinary_closeout' | 'start_unavailable';

function coerceRecordingReason(value: unknown): RecordingReason {
	...
	throw new ToolInputError('recording_reason must be one of: ordinary_closeout, start_unavailable.');
}
...
function buildFinishTaskRequestBindingHash(
	requestHash: string,
	context: ToolInvocationContext
): string {
	return crypto
...
function isWritebackConfirmationBinding(value: unknown): value is WritebackConfirmationBinding {
	...
	const hasAppliedProposalReferenceFlag =
		typeof value.taskHadAppliedProposalReference === 'boolean';
		...
		...(typeof binding.taskHadProposalIdReference === 'boolean'
			...
			? {
				...
				...(typeof binding.taskHadAppliedProposalReference === 'boolean'
					? {
						taskHadAppliedProposalReference:
							binding.taskHadAppliedProposalReference,
					}
					: {}),
			}
			: {}),
...
interface AgentTaskMetadata extends ResolvedProjectIdentity {
	...
	taskRecordOrigin: TaskRecordOrigin | '';
	startedAt: string;
...
function agentTaskMetadataFromFrontmatter(frontmatter: Record<string, unknown>): AgentTaskMetadata {
	...
	return {
		...
		].includes(taskRecordOrigin) ? taskRecordOrigin as TaskRecordOrigin : '',
		...
		startedAtSource: ['server_observed', 'client_claim'].includes(startedAtSource)
			? startedAtSource as StartedAtSource
			: 'unknown',
...
function normalizedTaskSourceCaptures(values: readonly string[], context: ToolContext): string[] {
	const captures = new Set<string>();
	for (const value of values) {
		try {
			const normalized = normalizeNotePath(value, pathSafetyOptions(context));
			if (isKnowledgeSourcePath(normalized)) {
...
function unresolvedDurableProposalSnapshot(
	proposalId: string,
	proposalPath: string
): FinishTaskDurableProposalSnapshot {
	return {
		proposalId,
		path: proposalPath,
		proposalKind: '',
...
async function snapshotExactTaskProposal(
	vaultRoot: string,
	taskId: string,
	proposalId: string,
	rawPath: string,
	...
	let proposalPath = '';
	try {
		const normalized = normalizeNotePath(rawPath, pathSafetyOptions(context));
	...
	try {
		...
		const document = memoryProposalDocumentFromText(
			vaultRoot,
			proposalPath,
			text,
			context
		);
		const explicitProposalId = stripYamlQuotes(
			readFrontmatterString(document.frontmatter, ['proposal_id', 'proposalId'])
		);
		if (
			!explicitProposalId
		...
		let targetPath = '';
		if (rawTargetPath && isAllowedProposalTargetPath(rawTargetPath)) {
			try {
				targetPath = normalizeNotePath(rawTargetPath, pathSafetyOptions(context));
			} catch {
				targetPath = '';
			}
		}
...
async function snapshotTaskDurableOutput(
	vaultRoot: string,
	taskId: string,
	metadata: AgentTaskMetadata,
	context: ToolContext
): Promise<FinishTaskDurableOutputSnapshot> {
	const proposals: FinishTaskDurableProposalSnapshot[] = [];
	...
	for (let index = 0; index < pairCount; index += 1) {
		...
		proposals.push(await snapshotExactTaskProposal(
			vaultRoot,
			taskId,
			proposalId,
			rawPath,
...
async function snapshotTaskAutoWrites(
	vaultRoot: string,
	taskId: string,
	metadata: AgentTaskMetadata,
	context: ToolContext
): Promise<NonNullable<FinishTaskDurableOutputSnapshot['autoWrites']>> {
	...
	for (const target of metadata.memoryWrites) {
		if (!target.startsWith(`${KNOWLEDGE_MEMORY_DIR}/`)) continue;
		const match = path.posix.basename(target).match(/^propose_memory-(propose-memory-[a-f0-9]{24})\.md$/);
		if (match) ids.add(match[1]);
	}
	const result: NonNullable<FinishTaskDurableOutputSnapshot['autoWrites']> = [];
	for (const operationId of ids) {
		let targetPath = '';
		let status: 'applied' | 'unresolved' = 'unresolved';
		try {
			if (!/^propose-memory-[a-f0-9]{24}$/.test(operationId)) throw new Error('Invalid auto-write identity.');
			...
			if (operation?.status === 'completed' && isRecord(payload) && isRecord(payload.requestSnapshot)
				&& payload.requestSnapshot.task_id === taskId && isRecord(receipt) && receipt.auto_applied === true
				&& typeof receipt.path === 'string' && metadata.memoryWrites.includes(receipt.path)
				&& isAllowedProposalTargetPath(receipt.path)) {
				targetPath = normalizeNotePath(receipt.path, pathSafetyOptions(context));
				if (await readVaultNoteContent(vaultRoot, targetPath, context) !== null) status = 'applied';
			}
		} catch {
...
async function updateManagedProposalReferences(
	...
	const nextFields: Record<string, ProposalFrontmatterMutationValue> = {
...
async function linkApprovedWritebackTask(
	...
	const usesAppliedProposalEvidence =
	...
	if (
		taskPlan.hadTargetReference !== payload.taskHadTargetReference
...
async function rollbackApprovedWritebackTask(
	...
	const appliedProposalEvidenceAdded =
		typeof payload.taskHadAppliedProposalReference === 'boolean'
	...
	const appliedProposalIds = readFrontmatterStringList(
		frontmatter,
		'durable_output_applied_proposal_ids'
	);
		...
		usesStableProposalReferences
			? {
				...
				...(typeof payload.taskHadAppliedProposalReference === 'boolean'
					? {
						durable_output_applied_proposal_ids:
							nextAppliedProposalIds.length > 0
...
async function createAgentTaskRecord(
	...
	started_at: string;
	started_at_source: 'server_observed' | 'client_claim';
	...
	const trackingMetadataFromContent = (content: string) => {
		...
		const trackingStartedAt = stripYamlQuotes(
			readFrontmatterString(fields, ['tracking_started_at'])
		) || startedAt;
		const recordedAt = stripYamlQuotes(readFrontmatterString(fields, ['recorded_at']))
			|| trackingStartedAt;
		const startedAtSource = stripYamlQuotes(
			readFrontmatterString(fields, ['started_at_source'])
		) === 'client_claim' ? 'client_claim' as const : 'server_observed' as const;
		return {
			tracking_mode: 'live' as const,
			task_record_origin: 'start_task' as const,
			started_at: startedAt,
			started_at_source: startedAtSource,
			tracking_started_at: trackingStartedAt,
			recorded_at: recordedAt,
			start_recovery: 'not_requested' as const,
		};
	};
	const returnExistingTask = async (content: string) => {
		const audit = await appendAuditEventAsync(vaultRoot, {
			operationId: input.operationId,
	...
	if (!existingTask && !input.context.vaultRepository) {
		...
		if (fs.existsSync(taskAbsolute)) {
			...
			const existing = parseMarkdown(existingContent);
	...
	const startedAtSource = input.clientStartedAt ? 'client_claim' as const : 'server_observed' as const;
...
function operationJournalForVault(vaultRoot: string, context?: ToolContext): NodeFileOperationJournal {
...
async function handleMemory(rawArgs: MemoryArgs, context: ToolInvocationContext) {
	...
	if (readView.index_state !== 'ready') {
		throw new ToolInputError(`knowledge_index_not_ready: ${readView.index_state}.`);
	}
	if (readView.errors.length > 0 || readView.memory.invalidPaths.length > 0) {
		throw new ToolInputError('memory_catalog_incomplete: unreadable or invalid records require inspection.');
	}
	if (scope === 'project') {
...
async function handleReadNote(rawArgs: ReadNoteArgs, context: ToolInvocationContext) {
	...
	if (rawArgs.expected_hash !== undefined && coerceOptionalString(rawArgs.expected_hash) !== contentHash) {
		throw new ToolInputError('note_content_changed: the note no longer matches the previous window.');
	}
...
function wikiBatchWritebackOverrideFor(
...
): { block: string; marker: string; previewNonce: string } | null {
	...
	if (!/^[a-f0-9]{32,64}$/.test(override.previewNonce)) {
		throw new ToolInputError('Wiki batch preview nonce is invalid.');
	}
...
function wikiBatchStableBindingHash(
	binding: WritebackConfirmationBinding,
	batchOperationId: string
): string {
 ...
 */
export async function previewObsidianWikiBatchWriteback(
	rawArgs: ApplyApprovedWritebackArgs,
	context: ToolInvocationContext
	...
	const batchOverride = wikiBatchWritebackOverrideFor(
		context,
		proposal.path,
		prepared.binding.targetPath,
		prepared.binding.effectKind
	);
	if (!batchOverride || !context.wikiBatchWritebackOverride) {
		throw new ToolInputError('Internal Wiki batch preview requires an Obsidian batch binding.');
	}
	return {
		...
		batch_stable_binding_hash: wikiBatchStableBindingHash(
			prepared.binding,
			context.wikiBatchWritebackOverride.batchOperationId
		),
...
async function handleLint(rawArgs: LintArgs, context: ToolInvocationContext) {
	...
	if (rawArgs.cursor !== undefined) {
		...
		const cursor = decodeMaintenanceCursor(rawCursor);
		if (cursor.generation !== view.generation) {
			throw new ToolInputError(`Maintenance cursor generation ${cursor.generation} is stale against ${view.generation}.`);
		}
		if (cursor.profile !== profile || cursor.page_size !== pageSize) {
			throw new ToolInputError('Maintenance cursor is invalid for the requested profile or page size.');
		}
	...
	const wikiMaintenanceCandidates = effectiveMaintenanceCandidates.filter((candidate) =>
		candidate.category === 'wiki_role' || candidate.category === 'wiki_relation'
	);
	const wikiProfileIssues = wikiMaintenanceCandidates
		.filter((candidate) => candidate.state !== 'informational')
		.map((candidate) => ({
			severity: (candidate.state === 'blocked' || profile === 'strict' ? 'error' : 'warning') as 'error' | 'warning',
	...
	const wikiRecommendations = wikiMaintenanceCandidates.map((candidate) =>
		candidate.category === 'wiki_role'
	...
	const maintenanceCursor = nextOffset < effectiveMaintenanceCandidates.length
		? encodeMaintenanceCursor({ version: 1, generation: view.generation, profile, page_size: pageSize, offset: nextOffset })
		: null;
...
async function loadSourceArchiveMetadataEvidence(
	...
	for (const archiveEntry of entries) {
		try {
			...
			const archiveItems = Array.isArray(archiveJournal.archive) ? archiveJournal.archive.filter(isRecord) : [];
			const outputs = Array.isArray(materialization.outputs) ? materialization.outputs.filter(isRecord) : [];
			const partsByLegacy = new Map<string, Array<{ path: string; expectedHash: string }>>();
			for (const output of outputs) {
				if (typeof output.path !== 'string' || typeof output.legacyPath !== 'string') continue;
				const parent = sourceIndexPathForPart(output.path);
				if (!parent) continue;
			...
			const outputsValid = outputs.every((output) =>
			...
			for (const archiveItem of archiveItems) {
				if (typeof archiveItem.oldPath !== 'string' || typeof archiveItem.destinationPath !== 'string') continue;
				const parts = partsByLegacy.get(archiveItem.oldPath) ?? [];
				const part = parts[0];
				const parent = part ? sourceIndexPathForPart(part.path) ?? '' : '';
				const parentEntry = view.catalog.get(parent);
				const manifest = parentEntry?.frontmatter.part_manifest;
				const partManifest = Array.isArray(manifest) ? manifest.filter((item): item is string => typeof item === 'string') : [];
				const archiveCatalog = view.catalog.get(archiveItem.destinationPath);
				if (!archiveCatalog) continue;
...
async function handleRequestMaintenance(
	rawArgs: RequestMaintenanceArgs,
	context: ToolInvocationContext,
) {
	...
	if (unexpected.length > 0) {
		throw new ToolInputError(`Unsupported maintenance request argument(s): ${unexpected.sort().join(', ')}.`);
	}
	const vaultRoot = configuredVaultRoot(context);
	if (
		typeof rawArgs.snapshot_generation !== 'number'
		|| !Number.isSafeInteger(rawArgs.snapshot_generation)
		|| rawArgs.snapshot_generation < 0
	) throw new ToolInputError('snapshot_generation must be a non-negative safe integer.');
	...
	if (candidateIds.length === 0 || candidateIds.length > 100) {
		throw new ToolInputError('candidate_ids must contain between 1 and 100 unique ids.');
	}
	if (candidateIds.some((candidateId) => !/^maintenance_[a-f0-9]{24}$/u.test(candidateId))) {
		throw new ToolInputError('candidate_ids contains an invalid maintenance candidate id.');
	}
	...
	if (idempotencyKey.length > 512) throw new ToolInputError('idempotency_key must be at most 512 characters.');
	if (taskId && taskId.length > 256) throw new ToolInputError('task_id must be at most 256 characters.');
	const requestId = `maintenance-request-${crypto.createHash('sha256').update(idempotencyKey).digest('hex').slice(0, 24)}`;
	const requestPath = `${TRACEKEEPER_AGENT_REQUESTS_DIR}/${requestId}.md`;
	const existing = await readVaultNoteContent(vaultRoot, requestPath, context);
	if (existing !== null) {
		const parsedRequest = parseMaintenanceRequestMarkdown(existing);
		if (!parsedRequest.valid) {
			throw new ToolInputError(`Existing maintenance request is invalid: ${parsedRequest.validationError}.`);
		}
		...
		if (
			...
			|| storedTaskId !== taskId
		) throw new ToolInputError('Idempotency conflict: maintenance request has a different input binding.');
		return {
			read_only: false,
			vault_root: vaultRoot,
	...
	if (taskId) {
		...
		if (taskContent === null || readFrontmatterString(parseMarkdown(taskContent).frontmatter.fields, ['task_id']) !== taskId) {
			throw new ToolInputError(`Maintenance request task is missing or does not match: ${taskId}.`);
		}
	}
	...
	if (view.source !== 'index' || view.generation !== rawArgs.snapshot_generation) {
		throw new ToolInputError(
			`Maintenance snapshot generation ${rawArgs.snapshot_generation} is stale against ${view.generation}.`
		);
	}
	...
	const selected = candidateIds.map((candidateId) => {
		...
		if (!item) throw new ToolInputError(`Maintenance candidate is missing or stale: ${candidateId}.`);
		if (!item.requestable || item.state === 'blocked') {
			throw new ToolInputError(`Maintenance candidate is not requestable: ${candidateId}.`);
		}
		return item;
	});
	const manifest = maintenanceRequestManifest(selected);
	const requestBindingHash = maintenanceRequestBindingHash({
		snapshot_generation: snapshot.generation,
		candidate_ids: candidateIds,
		task_id: taskId,
		manifest,
	});
	const record = await buildAndWriteNoteAsync(
		vaultRoot,
		...
		`${requestId}.md`,
		{
			...
			manifest_hash: maintenanceRequestManifestHash(manifest),
		...
		taskId,
		context,
		{ action: 'maintenance.request', candidate_count: candidateIds.length },
		requestId,
	);
	return {
		read_only: false,
...
function buildFinishTaskNextActions(
	...
	if (durableOutputNeedsReview(durableOutput)) {
		actions.push(contentText(
			context,
			`知识持久化状态为 ${durableOutput.status}；请在 Obsidian 的知识变更审核中处理。已捕获的 Source 或 Recall 结果不证明 Wiki/Memory 目标已写入。`,
			`Durable output is ${durableOutput.status}; handle it in Obsidian Knowledge Change Review. Captured Source or Recall results do not prove that the Wiki/Memory target was applied.`
		));
	}
	if (durableOutput.rejected_count > 0 || durableOutput.unresolved_count > 0) {
		actions.push(contentText(
			context,
			`知识持久化状态为 ${durableOutput.status}，请求的 Wiki/Memory 输出未持久化；Source 仍只作为来源证据。`,
			`Durable output is ${durableOutput.status}; the requested Wiki/Memory output was not persisted, while Source remains provenance evidence only.`
		));
	}
	if (actions.length === 0 && durableOutput.status === 'applied') {
		actions.push(contentText(
			context,
			'知识持久化状态为 applied；已应用的目标路径列在 durable_output.target_paths。',
			'Durable output is applied; the applied target paths are listed in durable_output.target_paths.'
		));
	} else if (actions.length === 0 && durableOutput.source_capture_count > 0) {
		actions.push(contentText(
			context,
			'已捕获的 Source 可作为来源证据读取，但没有 Wiki/Memory 输出被应用。不要将 Source 或 Recall 可读性描述为知识写入成功。',
			'Captured Source is readable provenance, but no Wiki/Memory output was applied. Do not describe Source or Recall readability as successful knowledge writeback.'
		));
	} else if (actions.length === 0) {
		actions.push(contentText(
			context,
			'任务追踪已记录；结束时没有关联 Wiki/Memory 持久化输出。',
			'Task tracking was recorded; no Wiki/Memory durable output was linked at finish.'
		));
	}
	if (projectHint) {
...
interface FinishTaskOperationPayload {
	...
	recordingReason: RecordingReason | null;
	startOperationId: string;
	summary: string;
	...
	durableOutputSnapshot?: FinishTaskDurableOutputSnapshot;
}

function isFinishTaskDurableOutputSnapshot(value: unknown): value is FinishTaskDurableOutputSnapshot {
	if (!isRecord(value)) {
		return false;
	}
	return Array.isArray(value.sourceCapturePaths)
		&& value.sourceCapturePaths.every((entry) => typeof entry === 'string')
		&& (value.autoWrites === undefined || (Array.isArray(value.autoWrites) && value.autoWrites.every((entry) =>
			isRecord(entry) && typeof entry.operationId === 'string' && typeof entry.path === 'string'
...
function isFinishTaskOperationPayload(payload: unknown): payload is FinishTaskOperationPayload {
	...
	if (
		!isRecord(payload.requestSnapshot)
		|| (
	...
	if (
		payload.taskRecordOrigin !== undefined
	...
	if (
		payload.startRecovery !== undefined
	...
	if (
		payload.durableOutputSnapshot !== undefined
...
function finishTaskTrackingMode(input: FinishTaskOperationPayload): TaskTrackingMode {
	return input.trackingMode === 'closeout_only' ? 'closeout_only' : 'live';
}

function finishTaskRecordOrigin(input: FinishTaskOperationPayload): TaskRecordOrigin {
	if (
		input.taskRecordOrigin === 'start_task'
...
function finishTaskRecordBindingHash(input: FinishTaskOperationPayload): string {
...
async function ensureFinishTaskRecordExists(
	...
	if (current) {
		assertFinishTaskRecordBinding(
			...
			input.taskId,
			operationId,
			finishTaskRecordBindingHash(input),
			false,
...
async function synchronizeFinishTaskTimestampFromRecord(
	...
	const fields = parseMarkdown(current.content).frontmatter.fields;
	const recordedAt = stripYamlQuotes(
		readFrontmatterString(fields, ['finish_recorded_at'])
	) || (finishTaskTrackingMode(input) === 'closeout_only'
...
async function markFinishTaskRecordClosing(
	...
	for (let attempt = 0; attempt < 2; attempt += 1) {
		...
		const currentLifecycle = await readTaskLifecycleStateAsync(
			input.vaultRoot,
			input.taskId,
			context
		);
		...
		try {
			...
			const lifecycle = await readTaskLifecycleStateAsync(
				input.vaultRoot,
				input.taskId,
				context
			);
...
function durableOutputSnapshotFromFinishPayload(
	input: FinishTaskOperationPayload
): FinishTaskDurableOutputSnapshot {
...
async function buildFinishTaskDurableOutput(
	input: FinishTaskOperationPayload,
	proposalResult: FinishTaskProposalResult,
	context: ToolContext
): Promise<FinishTaskDurableOutputEvidence> {
	...
	for (const proposal of proposalResult.proposals) {
		...
		const generated = await snapshotExactTaskProposal(
			input.vaultRoot,
			input.taskId,
			proposal.proposalId,
			proposal.path,
	...
	const counts: Record<DurableProposalStatus, number> = {
		pending_review: 0,
		ready_to_apply: 0,
	...
	for (const write of snapshot.autoWrites ?? []) {
		if (autoAppliedKeys.has(write.operationId)) continue;
		autoAppliedKeys.add(write.operationId);
		counts[write.status] += 1;
		if (write.status === 'unresolved') unresolvedAutoWriteCount += 1;
		if (write.path) targetPaths.add(write.path);
	}
	for (const update of proposalResult.autoAppliedMemoryUpdates) {
		...
		counts.applied += 1;
		try {
			const normalized = normalizeNotePath(update.path, pathSafetyOptions(context));
			if (isAllowedProposalTargetPath(normalized)) {
	...
	const activeStatuses = Object.entries(counts)
		.filter(([, count]) => count > 0)
		.map(([status]) => status as DurableProposalStatus);
	const status: DurableOutputStatus = activeStatuses.length === 0
		? 'none'
...
function aggregateFinishTaskProposalReferences(
	input: FinishTaskOperationPayload,
	proposalResult: FinishTaskProposalResult,
	sessionNotePath: string,
	context: ToolContext
): Array<ManagedProposalReference & { kind: string }> {
	const aggregated: Array<ManagedProposalReference & { kind: string }> = [];
	const seenIds = new Set<string>();
	const seenPaths = new Set<string>();
	const add = (proposal: ManagedProposalReference & { kind: string }) => {
		if (
			seenIds.has(proposal.proposalId)
...
function durableOutputFrontmatterFields(
	evidence: FinishTaskDurableOutputEvidence
...
function isFinishTaskDurableOutputEvidenceConsistent(
	evidence: FinishTaskDurableOutputEvidence,
	context: ToolContext
): boolean {
	const summary = evidence.summary;
	const stateCounts: Array<[DurableProposalStatus, number]> = [
	...
	const expectedStatus: DurableOutputStatus = activeStates.length === 0
		? 'none'
	...
	if (!Number.isSafeInteger(unresolvedAutoWriteCount) || unresolvedAutoWriteCount < 0 || unresolvedAutoWriteCount > summary.unresolved_count) return false;
	...
	if (
		!Number.isSafeInteger(nonAppliedProposalCount)
	...
	const uniqueList = (values: readonly string[]): boolean =>
		new Set(values).size === values.length;
	if (
		!uniqueList(evidence.proposalIdsAtFinish)
	...
	const exactNonAppliedCount = summary.pending_review_count
		...
		+ summary.rejected_count;
	if (
		evidence.proposalIdsAtFinish.length < exactNonAppliedCount
	...
	if (evidence.proposalIdsAtFinish.some(
		(proposalId) => proposalId.length === 0 || proposalId.length > 512
	)) {
		return false;
	}
	const exactNormalizedPath = (value: string): string | null => {
		try {
			const normalized = normalizeNotePath(value, pathSafetyOptions(context));
	...
	return summary.target_paths.every((targetPath) => {
		...
		return Boolean(normalized && isAllowedProposalTargetPath(normalized));
	});
}

function durableOutputFromFrontmatter(
	frontmatter: Record<string, unknown>,
	context: ToolContext
): FinishTaskDurableOutputEvidence | null {
	const requiredKeys = [
	...
	const evidence: FinishTaskDurableOutputEvidence = {
		...
		summary: {
			status,
			source_capture_count: counts.source_capture_count as number,
			proposal_count: counts.proposal_count as number,
			pending_review_count: counts.pending_review_count as number,
			ready_to_apply_count: counts.ready_to_apply_count as number,
			revision_requested_count: counts.revision_requested_count as number,
			applied_count: counts.applied_count as number,
			rejected_count: counts.rejected_count as number,
			unresolved_count: counts.unresolved_count as number,
		...
		proposalIdsAtFinish: readFrontmatterStringList(
			frontmatter,
			'durable_output_proposal_ids_at_finish'
		),
	};
...
type FinishTaskDurableOutputEvidenceReadMode = 'repair' | 'finalize';
...
async function updateManagedRecordFields(
	vaultRoot: string,
	recordPath: string,
	fields: Record<string, string | null>,
	context: ToolContext
	...
	const current = await readCurrentVaultTextState(vaultRoot, recordPath, context);
	if (!current) {
		throw new OperationConflictError(`Tracekeeper-managed record is unavailable: ${recordPath}`);
	}
	...
	if (context.vaultRepository) {
		if (!current.version) {
			throw new OperationConflictError(`Tracekeeper-managed record version is unavailable: ${recordPath}`);
		}
		await context.vaultRepository.replaceText(current.path, current.version, next);
		return;
	}
...
function buildFinishTaskRequestSnapshot(rawArgs: FinishTaskArgs) {
	...
	if (taskId && hasCloseoutOnlyField) {
		throw new ToolInputError(
			'goal, started_at, recording_reason, and start_idempotency_key must be omitted when task_id is provided.'
		);
	}
	...
	if (recordingReason === 'start_unavailable' && !startIdempotencyKey) {
		throw new ToolInputError('start_idempotency_key is required when recording_reason is start_unavailable.');
	}
	if (recordingReason === 'ordinary_closeout' && startIdempotencyKey) {
		throw new ToolInputError('start_idempotency_key is allowed only when recording_reason is start_unavailable.');
	}
	if (startIdempotencyKey.length > 512) {
		throw new ToolInputError('start_idempotency_key must be at most 512 characters.');
	}
	if (!taskId && !coerceOptionalString(rawArgs.idempotency_key)) {
		throw new ToolInputError('idempotency_key is required when task_id is omitted.');
	}
	if (!taskId && (rawArgs.status === undefined || rawArgs.status === null || rawArgs.status === '')) {
		throw new ToolInputError('status is required when task_id is omitted.');
	}
	return {
		task_id: taskId || null,
		goal: goal || null,
...
interface ResolvedFinishTaskRecording {
	taskId: string;
	goal: string;
...
function assertRecoveredStartPayloadMatches(
	payload: Record<string, unknown>,
	goal: string,
	startedAt: string,
	client: string,
	explicitIdentity: ResolvedProjectIdentity
): void {
	if (coerceOptionalString(payload.goal) !== goal) {
		throw new OperationConflictError('Original start_task goal does not match the closeout goal.');
	}
	const startClient = coerceOptionalString(payload.client);
	if (startClient && client && startClient !== client) {
		throw new OperationConflictError('Original start_task client does not match the closeout client.');
	}
	...
	if (originalStartedAt && coerceIsoTimestamp(originalStartedAt, true, 'started_at') !== startedAt) {
		throw new OperationConflictError('Original start_task started_at does not match the closeout started_at.');
	}
	for (const [field, startValue, finishValue] of [
		...
		if (!projectIdentityValueMatches(field, startValue, finishValue)) {
			throw new OperationConflictError(
				`Original start_task ${field} does not match the closeout request.`
			);
		}
	}
}

function finishRecordingFromTaskMetadata(
	taskId: string,
	metadata: AgentTaskMetadata,
	projectIdentity: ResolvedProjectIdentity,
	client: string,
...
async function resolveFinishTaskRecording(
	rawArgs: FinishTaskArgs,
	context: ToolInvocationContext,
	operationId: string,
	explicitIdentity: ResolvedProjectIdentity,
	finishedAt: string
): Promise<ResolvedFinishTaskRecording> {
	const vaultRoot = configuredVaultRoot(context);
	...
	if (explicitTaskId) {
		...
		return finishRecordingFromTaskMetadata(
			explicitTaskId,
			metadata,
			projectIdentity,
			requestedClient,
	...
	if (recordingReason === 'start_unavailable') {
		const startIdempotencyKey = coerceNonEmptyString(
			rawArgs.start_idempotency_key,
			true,
		...
		let taskState = await readCurrentVaultTextState(
			vaultRoot,
			buildTaskNotePath(expectedTaskId),
			context
		);
		let startRecord = await operationJournalForVault(vaultRoot, context).loadById(startOperationId);
		if (taskState) {
			const metadata = agentTaskMetadataFromFrontmatter(
				parseMarkdown(taskState.content).frontmatter.fields
			);
			if (metadata.startOperationId !== startOperationId) {
				throw new OperationConflictError('Recovered task is not bound to the supplied start_idempotency_key.');
			}
			if (metadata.objective && metadata.objective !== goal) {
				throw new OperationConflictError('Recovered task goal does not match the closeout goal.');
			}
			if (metadata.client && requestedClient && metadata.client !== requestedClient) {
				throw new OperationConflictError('Recovered task client does not match the closeout client.');
			}
			if (
				metadata.startedAt
				...
				throw new OperationConflictError('Recovered task started_at does not match the closeout started_at.');
			}
		...
		if (startRecord) {
			if (startRecord.idempotency_key !== startIdempotencyKey || !isRecord(startRecord.payload)) {
				throw new OperationConflictError('Original start_task journal binding is incompatible.');
			}
			assertRecoveredStartPayloadMatches(
				startRecord.payload,
				goal,
				startedAt,
				requestedClient,
			...
			taskState = await readCurrentVaultTextState(
				vaultRoot,
				buildTaskNotePath(expectedTaskId),
				context
			);
			...
			const metadata = taskState
				? agentTaskMetadataFromFrontmatter(parseMarkdown(taskState.content).frontmatter.fields)
				: emptyAgentTaskMetadata();
			const startPayload = isRecord(startRecord?.payload) ? startRecord.payload : {};
			...
			return finishRecordingFromTaskMetadata(
				...
				metadata,
				projectIdentity,
				requestedClient || coerceOptionalString(startPayload.client),
				'matched',
				{
					goal,
...
async function buildFinishTaskOperationPayload(
	...
	const recording = await resolveFinishTaskRecording(
		rawArgs,
		context,
		operationId,
		explicitIdentity,
	...
	const durableOutputSnapshot = await snapshotTaskDurableOutput(
		vaultRoot,
		taskId,
		taskMetadata,
		context
...
async function executeFinishTaskOperation(
	...
	if (!durableOutputEvidence) {
		throw new OperationConflictError(
			'Finish-task durable-output evidence is unavailable at finalization.'
		);
	}
	...
	const memoryCloseoutState = compatibleMemoryCloseoutStatus(
		durableOutput,
		resolveCanonicalMemoryCloseoutStatus(
			input.reviewProposalMode,
			proposalResult,
			input.hasCloseoutCandidates,
			resolveMemoryCloseoutStatus(
				input.reviewProposalMode,
				proposalResult,
				input.hasCloseoutCandidates
			)
	...
	const response: {
		...
		durable_output: FinishTaskDurableOutputSummary;
		project_id?: string | null;
		repo_path?: string | null;
		project_hint?: string | null;
...
async function prepareAndFinishTask(rawArgs: FinishTaskArgs, context: ToolInvocationContext) {
	const vaultRoot = configuredVaultRoot(context);
	const invocationContext = { ...context };
	const readView = await knowledgeReadViewForContext(vaultRoot, invocationContext);
	const preflightScan = lightweightScanFromReadView(vaultRoot, readView);
