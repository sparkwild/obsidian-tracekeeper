...
interface Copy {
	localBadge: string;
	footer: string;
	waiting: { title: string; lead: string; clientLabel: string; expiresLabel: string; refreshLabel: string };
	errors: Record<OAuthPageErrorKind, { title: string; message: string }>;
}

const COPY: Record<OAuthUiLocale, Copy> = {
	'zh-CN': {
		localBadge: '本机安全连接',
...
const OAUTH_PAGE_STYLE_HASH = createHash('sha256').update(OAUTH_PAGE_CSS, 'utf8').digest('base64');

export function normalizeOAuthUiLocale(value: unknown): OAuthUiLocale { return value === 'en' ? 'en' : 'zh-CN'; }
...
export function renderOAuthWaitingPage(locale: OAuthUiLocale, model: OAuthWaitingPageModel): string {
...
export function renderOAuthErrorPage(locale: OAuthUiLocale, kind: OAuthPageErrorKind): string {
	const copy = COPY[locale];
