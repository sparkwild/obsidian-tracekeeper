...
export type OAuthPageErrorKind =
	| 'invalid_request'
	| 'invalid_client'
...
interface OAuthPageCopy {
	...
	error: {
		footer: string;
		states: Record<OAuthPageErrorKind, { title: string; message: string }>;
	};
}

const COPY: Record<OAuthUiLocale, OAuthPageCopy> = {
	'zh-CN': {
		localBadge: '本机安全连接',
...
const OAUTH_PAGE_STYLE_HASH = createHash('sha256')
	.update(OAUTH_PAGE_CSS, 'utf8')
	.digest('base64');

export function normalizeOAuthUiLocale(value: unknown): OAuthUiLocale {
	return value === 'en' ? 'en' : 'zh-CN';
}

export function oauthContentSecurityPolicy(callbackOrigin?: string): string {
	return [
		"default-src 'none'",
		"base-uri 'none'",
...
export function renderOAuthPairingPage(
	locale: OAuthUiLocale,
	model: OAuthPairingPageModel
): string {
...
export function renderOAuthConfirmationPage(
	locale: OAuthUiLocale,
	model: OAuthConfirmationPageModel
): string {
	const copy = COPY[locale];
	return pageShell(
		locale,
...
export function renderOAuthErrorPage(
	...
	return pageShell(
		locale,
		state.title,
		`<section class="oauth-card oauth-error" role="alert" aria-labelledby="oauth-page-title">
...
function pageShell(locale: OAuthUiLocale, title: string, card: string): string {
	const copy = COPY[locale];
	return `<!doctype html>
<html lang="${locale}">
<head>
...
function renderHiddenInputs(fields: ReadonlyArray<readonly [string, string]>): string {
	return fields
