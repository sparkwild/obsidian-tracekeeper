...
exports.ProposalWritebackFormatError = void 0;
exports.renderProposalWritebackSection = renderProposalWritebackSection;
exports.parseProposalWritebackBody = parseProposalWritebackBody;
exports.resolveProposalWriteback = resolveProposalWriteback;
exports.replaceProposalWriteback = replaceProposalWriteback;
...
class ProposalWritebackFormatError extends Error {
    constructor(message) {
        super(message);
        this.name = 'ProposalWritebackFormatError';
    }
}
exports.ProposalWritebackFormatError = ProposalWritebackFormatError;
const normalizeMarkdown = (value) => value.replace(/\r\n?/g, '\n');
const normalizePayload = (value) => normalizeMarkdown(value).trim();
const assertProposalId = (proposalId) => {
...
const markerMatches = (body, pattern) => {
    const matches = [];
    pattern.lastIndex = 0;
    let match;
...
const parseBoundedWriteback = (body, proposalId) => {
...
const parseLegacyWriteback = (body) => {
...
function renderProposalWritebackSection(heading, proposalId, content) {
    ...
    if (markerMatches(payload, START_MARKER_PATTERN).length > 0
...
function parseProposalWritebackBody(body, proposalId) {
    const normalized = normalizeMarkdown(body);
    return parseBoundedWriteback(normalized, proposalId)
        ?? parseLegacyWriteback(normalized);
}
function resolveProposalWriteback(input) {
...
function replaceProposalWriteback(body, proposalId, content) {
    const normalized = normalizeMarkdown(body);
    ...
    if (parsed.format === 'bounded_v2') {
        ...
        replaced = `${normalized.slice(0, parsed.headingStart)}${renderProposalWritebackSection(heading, proposalId, payload)}`;
    }
