...
const hashText = (text) => node_crypto_1.default.createHash('sha256').update(text).digest('hex');
const readFrontmatterStringList = (fields, key) => fields.task_record_version !== undefined && Array.isArray(fields[key]) ? fields[key].map(String) : (0, task_record_1.legacyTaskReferenceList)(fields[key]);
function mergeFrontmatterList(fields, key, values) { return [...new Set([...readFrontmatterStringList(fields, key), ...values.map((value) => value.trim()).filter(Boolean)])].join(', '); }
/**
...
function planApprovedWritebackTaskLink(input) {
    const original = (0, markdown_1.parseMarkdown)(input.taskContent).frontmatter.fields;
    const task = (0, task_record_1.parseTaskRecord)(original);
    const frontmatter = (0, task_record_1.taskReferenceFields)(original);
    ...
    if (task) {
        const target = (0, task_record_1.makeTaskRelation)({ taskId: task.task_id, role: 'written_output', path: input.targetPath, operationId: input.operationId });
        ...
        const proposal = (0, task_record_1.makeTaskRelation)({ taskId: task.task_id, role: 'review_proposal', path: input.proposalPath, proposalId: input.proposalId });
        let content = (0, task_record_1.updateTaskRelations)(input.taskContent, [target, ...(!task.relations.some((row) => row.role === 'review_proposal' && row.target_id === input.proposalId) ? [proposal] : [])]);
        if (input.usesAppliedProposalEvidence && !hadAppliedProposalReference)
            content = (0, task_record_1.patchTaskMetadata)(content, { durable_output_applied_proposal_ids: [...appliedProposalIds, input.proposalId] });
