...
function extractEvidenceCandidates(text, language) {
...
function extractClaimCandidates(text, language) {
...
function buildSummary(source, analysisMode, purpose, text, language) {
    ...
    if (!cleanText) {
        return localizedText(language, `未捕获 ${sourceHint} 的内联正文。${purposeHint}。${modeHint}。`, `No inline text was captured for ${sourceHint}${purposeHint}. ${modeHint}.`);
    }
...
function buildProposalDrafts(source, sourceKind, analysisMode, summary, evidences, claims, requestPath, language) {
    const drafts = [];
    ...
    drafts.push({
        ...
        content: `${localizedText(language, '## 提案草稿', '## Proposal draft')}\n\n` +
...
function analyzeSourceText(input) {
    ...
    const contentLanguage = input.contentLanguage === 'zh-CN' ? 'zh-CN' : 'en';
    ...
    if (IMPORTANT_HINT_RE.test(content) && evidenceScaffolds.length === 0) {
        evidenceScaffolds.push(localizedText(contentLanguage, '证据：重新扫描来源中的具体引用或链接。', 'evidence: Re-scan source for concrete references or links.'));
    }
    return {
        summary,
