...
import { KNOWLEDGE_PROJECTS_MEMORY_DIR } from './knowledge-architecture';
import { normalizeVaultRelativePath } from './knowledge-note';

export const PROJECT_MEMORY_ENTRY_SCHEMA_VERSION = 1 as const;
export const PROJECT_MEMORY_ENTRY_TYPE = 'project_memory_entry' as const;
export const PROJECT_MEMORY_HUB_TYPE = 'project_memory_index' as const;
export const PROJECT_MEMORY_CATALOG_SORT =
	'created_at_desc_operation_id_path_asc' as const;
export const PROJECT_MEMORY_DEFAULT_PAGE_SIZE = 50;
export const PROJECT_MEMORY_MAX_PAGE_SIZE = 200;
...
const PROJECT_ENTRY_PATH_PATTERN = new RegExp(
	`^${escapeRegularExpression(KNOWLEDGE_PROJECTS_MEMORY_DIR)}/([^/]+)/agents/([^/]+)/[^/]+\\.md$`
);
const PROJECT_LEGACY_PATH_PATTERN = new RegExp(
	`^${escapeRegularExpression(KNOWLEDGE_PROJECTS_MEMORY_DIR)}/([^/]+)/memory\\.md$`
);
const PROJECT_HUB_PATH_PATTERN = new RegExp(
	`^${escapeRegularExpression(KNOWLEDGE_PROJECTS_MEMORY_DIR)}/([^/]+)/index\\.md$`
);

export type ProjectMemoryEntryStatus =
	| 'active'
	| 'superseded'
	| 'disputed'
	| 'review';
...
export interface BuildProjectMemoryEntryInput
	extends ProjectMemoryOperationHashInput {
	project_key: string;
	created_at: string;
}
...
interface ProjectMemoryCursorPayload {
	v: 1;
	project_id: string;
	generation: number;
	sort: typeof PROJECT_MEMORY_CATALOG_SORT;
...
interface ProjectMemoryCursorEnvelope extends ProjectMemoryCursorPayload {
	checksum: string;
}
...
export class ProjectMemoryCursorError extends Error {
...
export class StaleProjectMemoryCursorError extends Error {
	...
	constructor(cursorGeneration: number, currentGeneration: number) {
		super(
			`Project-memory cursor generation ${cursorGeneration} is stale; current generation is ${currentGeneration}.`
		);
...
export function normalizeProjectRepositoryPath(value: string): string {
	...
	if (/^file:/i.test(normalized)) {
		...
		if (
			...
			throw new ProjectMemoryValidationError(
				'repo_path file URL must identify a local path.'
			);
		}
		try {
			...
			throw new ProjectMemoryValidationError(
				'repo_path file URL contains invalid escaping.'
			);
		}
	...
	if (
		...
		throw new ProjectMemoryValidationError(
			'repo_path must be an exact absolute path.'
		);
	}
	const segments = normalized.split('/');
	if (segments.some((segment) => segment === '..')) {
		throw new ProjectMemoryValidationError(
			'repo_path must not contain traversal segments.'
		);
	}
	if (segments.some((segment) => segment === '.')) {
		throw new ProjectMemoryValidationError(
			'repo_path must not contain relative path segments.'
		);
	}
	...
	if (normalized === '/' || /^[A-Za-z]:\/$/.test(normalized)) {
		throw new ProjectMemoryValidationError(
			'repo_path must identify a repository below the filesystem root.'
		);
	}
	return normalized;
}

export function deriveProjectIdentityFromRepoPath(
	repoPath: string
...
export function deriveProjectMemoryHubBindingFromRepoPath(
	repoPath: string
): ProjectMemoryHubBinding & { project_hint: string } {
	const identity = deriveProjectIdentityFromRepoPath(repoPath);
	return {
		project_id: identity.project_id,
		project_key: identity.project_key,
		project_hub:
			`${KNOWLEDGE_PROJECTS_MEMORY_DIR}/${identity.project_key}/index.md`,
		repo_path: identity.repo_path,
		project_hint: identity.project_hint,
	};
}

export function normalizeProjectAgentType(value: unknown): string {
...
export function buildProjectMemoryEntryPath(
	input: ProjectMemoryEntryPathInput
): string {
	...
	return normalizeVaultRelativePath(
		`${KNOWLEDGE_PROJECTS_MEMORY_DIR}/${projectKey}/agents/${agentType}/${operationKind}-${operationId}.md`
	);
}

export function computeProjectMemoryOperationHash(
	input: ProjectMemoryOperationHashInput
): string {
...
export function compareProjectMemoryOperationHashes(
	existingOperationHash: string,
	requestedOperationHash: string
): ProjectMemoryOperationHashComparison {
	const existing = requireOperationHash(
		existingOperationHash,
		'existing operation_hash'
	);
	const requested = requireOperationHash(
		requestedOperationHash,
		'requested operation_hash'
	);
...
export function buildProjectMemoryEntry(
	input: BuildProjectMemoryEntryInput
): BuiltProjectMemoryEntry {
...
export function parseProjectMemoryEntry(
	source: ProjectMemorySourceNote
): ProjectMemoryEntry {
	const notePath = normalizeVaultRelativePath(source.path);
	const pathMatch = PROJECT_ENTRY_PATH_PATTERN.exec(notePath);
	if (!pathMatch) {
		throw new ProjectMemoryValidationError(
			`Project-memory entry path is invalid: ${notePath}`
		);
	}
	const projectKey = requireProjectKey(pathMatch[1]);
	const frontmatter = requirePlainObject(source.frontmatter, 'frontmatter');
	if (frontmatter.schema_version !== PROJECT_MEMORY_ENTRY_SCHEMA_VERSION) {
		throw new ProjectMemoryValidationError(
			`project-memory schema_version must be ${PROJECT_MEMORY_ENTRY_SCHEMA_VERSION}.`
		);
	}
	if (frontmatter.type !== PROJECT_MEMORY_ENTRY_TYPE) {
		throw new ProjectMemoryValidationError(
			`project-memory type must be ${PROJECT_MEMORY_ENTRY_TYPE}.`
		);
	}
	...
	const memoryKinds = normalizeIdentifierList(
		frontmatter.memory_kinds,
		'memory_kinds',
	...
	const operationHash = requireOperationHash(
		frontmatter.operation_hash,
		'operation_hash'
	);
	const projectHub = normalizeInternalLink(
		frontmatter.project_hub,
		'project_hub'
	);
	const relatedWiki = normalizeLinkList(
		frontmatter.related_wiki,
		'related_wiki'
	);
	const supersedes = normalizeLinkList(
	...
	if (expectedPath !== notePath) {
		throw new ProjectMemoryValidationError(
			`Project-memory entry path conflicts with its operation identity: ${notePath}`
		);
	}
	const expectedHub = `[[${KNOWLEDGE_PROJECTS_MEMORY_DIR}/${projectKey}/index]]`;
	if (projectHub !== expectedHub) {
		throw new ProjectMemoryValidationError(
			`project_hub conflicts with the entry project key: ${projectHub}`
		);
	}
...
export function parseProjectMemoryHub(
	source: ProjectMemorySourceNote
	...
	if (!pathMatch) {
		throw new ProjectMemoryValidationError(
			`Project-memory hub path is invalid: ${notePath}`
		);
	}
	...
	if (frontmatter.schema_version !== PROJECT_MEMORY_ENTRY_SCHEMA_VERSION) {
		throw new ProjectMemoryValidationError(
			`project-memory hub schema_version must be ${PROJECT_MEMORY_ENTRY_SCHEMA_VERSION}.`
		);
	}
	if (frontmatter.type !== PROJECT_MEMORY_HUB_TYPE) {
		throw new ProjectMemoryValidationError(
			`project-memory hub type must be ${PROJECT_MEMORY_HUB_TYPE}.`
		);
	}
	const projectId = requireProjectId(frontmatter.project_id);
	const projectKey = requireProjectKey(frontmatter.project_key);
	const repoPath = normalizeProjectRepositoryPath(
		requireString(frontmatter.repo_path, 'repo_path')
	);
	if (projectKey !== pathProjectKey) {
		throw new ProjectMemoryValidationError(
			`project_key conflicts with the hub path: ${projectKey}`
		);
	}
	return {
		project_id: projectId,
...
export function classifyProjectMemoryNote(
	source: ProjectMemorySourceNote
): ProjectMemoryNoteClassification {
	const notePath = normalizeVaultRelativePath(source.path);
	...
	if (hubMatch) {
		...
		if (
			source.frontmatter.schema_version !== PROJECT_MEMORY_ENTRY_SCHEMA_VERSION
			|| source.frontmatter.type !== PROJECT_MEMORY_HUB_TYPE
...
export function validateProjectMemoryOwnership(
	bindings: readonly ProjectMemoryHubBinding[]
): ProjectMemoryHubBinding[] {
	const normalized: ProjectMemoryHubBinding[] = [];
	...
	for (const binding of bindings) {
		...
		const projectHub = normalizeVaultRelativePath(binding.project_hub);
		const repoPath = normalizeProjectRepositoryPath(binding.repo_path);
		const expectedHub = `${KNOWLEDGE_PROJECTS_MEMORY_DIR}/${projectKey}/index.md`;
		if (projectHub !== expectedHub) {
			throw new ProjectMemoryValidationError(
				`project_hub conflicts with project_key ${projectKey}.`
			);
		}
		if (projectIds.has(projectId)) {
			throw new ProjectMemoryValidationError(
				`Project-memory ownership contains duplicate project_id ${projectId}.`
			);
		}
		if (projectKeys.has(projectKey)) {
			throw new ProjectMemoryValidationError(
				`Project-memory ownership contains duplicate project_key ${projectKey}.`
			);
		}
		if (repoPaths.has(repoPath)) {
			throw new ProjectMemoryValidationError(
				`Project-memory ownership contains duplicate repo_path ${repoPath}.`
			);
		}
...
export function resolveProjectMemoryNoteOwnership(
	...
	const byKey = validated.find(
		(binding) => binding.project_key === classification.project_key
	);
	const byId = classification.project_id
		? validated.find(
				(binding) => binding.project_id === classification.project_id
		  )
		: null;
	if (byKey && byId && byKey.project_id !== byId.project_id) {
		throw new ProjectMemoryValidationError(
			`Project-memory note ownership is ambiguous because project_id conflicts with its path: ${classification.path}`
		);
	}
	const binding = byKey ?? byId;
	if (!binding) {
		throw new ProjectMemoryValidationError(
			`Project-memory note ownership is ambiguous or missing: ${classification.path}`
		);
	}
	if (
		...
		throw new ProjectMemoryValidationError(
			`Project-memory note ownership conflicts with project_id: ${classification.path}`
		);
	}
	if (binding.project_key !== classification.project_key) {
		throw new ProjectMemoryValidationError(
			`Project-memory note ownership conflicts with project_key: ${classification.path}`
		);
	}
	return binding;
}

export function projectMemoryCatalogEntryFromClassification(
...
export function buildProjectMemoryCatalogPage(
	input: ProjectMemoryCatalogPageInput
): ProjectMemoryCatalogPage {
	const projectId = requireProjectId(input.projectId);
	const projectHub = normalizeVaultRelativePath(input.projectHub);
	const hubMatch = PROJECT_HUB_PATH_PATTERN.exec(projectHub);
	if (!hubMatch) {
		throw new ProjectMemoryValidationError(
			`Project-memory catalog hub path is invalid: ${projectHub}`
		);
	}
	const projectKey = requireProjectKey(hubMatch[1]);
	...
	if (offset > entries.length) {
		throw new ProjectMemoryCursorError(
			'Project-memory cursor offset is beyond the current catalog.'
		);
	}
	...
	const nextCursor =
			...
			? encodeProjectMemoryCursor({
					...
					project_id: projectId,
					generation,
					sort: PROJECT_MEMORY_CATALOG_SORT,
					offset: end,
			  })
			: null;
...
function canonicalProjectMemoryOperation(
	input: ProjectMemoryOperationHashInput
): Record<string, unknown> {
	if (!input || typeof input !== 'object' || Array.isArray(input)) {
		throw new ProjectMemoryValidationError(
			'Project-memory operation payload must be an object.'
		);
	}
	...
	return {
		...
		memory_kinds: normalizeIdentifierList(
			input.memory_kinds,
			'memory_kinds',
			OPERATION_KIND_PATTERN,
			true
...
function normalizeCatalogEntries(
	input: readonly ProjectMemoryCatalogEntry[],
	projectId: string,
	projectKey: string
): ProjectMemoryCatalogEntry[] {
	...
	const entries: ProjectMemoryCatalogEntry[] = [];
	...
	for (const candidate of input) {
		...
		if (paths.has(entry.path)) {
			throw new ProjectMemoryValidationError(
				`Project-memory catalog contains duplicate path ${entry.path}.`
			);
		}
		...
		if (!entry.legacy) {
			...
			if (operationIdentities.has(identity)) {
				throw new ProjectMemoryValidationError(
					`Project-memory catalog contains duplicate operation identity ${entry.operation_id}.`
				);
			}
...
function normalizeCatalogEntry(
	candidate: ProjectMemoryCatalogEntry,
	projectId: string,
	projectKey: string
): ProjectMemoryCatalogEntry {
	if (!candidate || typeof candidate !== 'object' || Array.isArray(candidate)) {
		throw new ProjectMemoryValidationError(
			'Project-memory catalog entry must be an object.'
		);
	}
	const entryProjectId = requireProjectId(candidate.project_id);
	if (entryProjectId !== projectId) {
		throw new ProjectMemoryValidationError(
			`Project-memory catalog entry belongs to another project_id: ${entryProjectId}`
		);
	}
	const notePath = normalizeVaultRelativePath(candidate.path);
	if (candidate.legacy) {
		const expectedLegacyPath =
			`${KNOWLEDGE_PROJECTS_MEMORY_DIR}/${projectKey}/memory.md`;
		if (notePath !== expectedLegacyPath) {
			throw new ProjectMemoryValidationError(
				`Legacy project-memory entry path conflicts with the catalog project: ${notePath}`
			);
		}
		if (
			candidate.agent_type !== null
			...
			throw new ProjectMemoryValidationError(
				'Legacy project-memory catalog fields must be null.'
			);
		}
		return {
			path: notePath,
	...
	if (!pathMatch) {
		throw new ProjectMemoryValidationError(
			`Project-memory catalog entry path is invalid: ${notePath}`
		);
	}
	const entryProjectKey = requireProjectKey(pathMatch[1]);
	if (entryProjectKey !== projectKey) {
		throw new ProjectMemoryValidationError(
			`Project-memory catalog entry belongs to another project_key: ${entryProjectKey}`
		);
	}
	...
	if (expectedPath !== notePath) {
		throw new ProjectMemoryValidationError(
			`Project-memory catalog path conflicts with operation identity: ${notePath}`
		);
	}
	return {
		path: notePath,
		...
		operation_hash: requireOperationHash(
...
function compareCatalogEntries(
	left: ProjectMemoryCatalogEntry,
	right: ProjectMemoryCatalogEntry
): number {
...
function countCatalogEntriesByAgent(
	entries: readonly ProjectMemoryCatalogEntry[]
...
function encodeProjectMemoryCursor(payload: ProjectMemoryCursorPayload): string {
	const checksum = hashCursorPayload(payload);
	const envelope: ProjectMemoryCursorEnvelope = {
		...payload,
		checksum,
	};
...
function decodeProjectMemoryCursor(
	cursor: string,
	projectId: string,
	generation: number
): number {
	if (
		...
		throw new ProjectMemoryCursorError(
			'Project-memory cursor encoding is invalid.'
		);
	}
	let decoded: string;
	try {
	...
	try {
		...
		throw new ProjectMemoryCursorError(
			'Project-memory cursor payload is invalid.'
		);
	}
	...
	if (
		...
		throw new ProjectMemoryCursorError(
			'Project-memory cursor shape is invalid.'
		);
	}
	if (
		parsed.v !== 1
		...
		|| parsed.generation < 0
		|| parsed.sort !== PROJECT_MEMORY_CATALOG_SORT
		...
		throw new ProjectMemoryCursorError(
			'Project-memory cursor fields are invalid.'
		);
	}
	const payload: ProjectMemoryCursorPayload = {
		v: 1,
	...
	if (parsed.checksum !== hashCursorPayload(payload)) {
		throw new ProjectMemoryCursorError(
			'Project-memory cursor checksum is invalid.'
		);
	}
	if (payload.project_id !== projectId) {
		throw new ProjectMemoryCursorError(
			'Project-memory cursor belongs to another project.'
		);
	}
...
function hashCursorPayload(payload: ProjectMemoryCursorPayload): string {
...
function stableJson(value: unknown): string {
	...
	if (
		...
		if (typeof value === 'number' && !Number.isFinite(value)) {
			throw new ProjectMemoryValidationError(
				'Canonical project-memory values must be finite.'
			);
		}
	...
	throw new ProjectMemoryValidationError(
		'Canonical project-memory values must be JSON-compatible.'
	);
}
...
function requireAgentType(value: unknown): string {
	return requireIdentifier(value, AGENT_TYPE_PATTERN, 'agent_type');
}

function requireOperationKind(value: unknown): string {
...
function normalizeIdentifierList(
	...
	const normalized = [
		...new Set(
			value.map((item) => requireIdentifier(item, pattern, `${name} item`))
		),
	].sort();
...
function normalizeLinkList(value: unknown, name: string): string[] {
	if (!Array.isArray(value)) {
		throw new ProjectMemoryValidationError(`${name} must be an array.`);
	}
	return [
...
function normalizeInternalLink(value: unknown, name: string): string {
	const raw = requireString(value, name);
	const match = /^\[\[([^|\]]+)(?:\|[^\]]+)?\]\]$/.exec(raw.trim());
	if (!match) {
		throw new ProjectMemoryValidationError(
			`${name} must be an internal Wikilink.`
		);
	}
	...
	const pathPart =
		subpathIndex >= 0
			...
			: targetWithSubpath;
	const subpath =
		subpathIndex >= 0 ? targetWithSubpath.slice(subpathIndex) : '';
	const normalizedPath = normalizeVaultRelativePath(
		pathPart.replace(/\.md$/i, '')
	);
	return `[[${normalizedPath}${subpath}]]`;
}

function requireProjectMemoryStatus(value: unknown): ProjectMemoryEntryStatus {
	if (
		value !== 'active'
		&& value !== 'superseded'
		&& value !== 'disputed'
		...
		throw new ProjectMemoryValidationError(
			'status must be active, superseded, disputed, or review.'
		);
	}
	return value;
}
...
function requireOperationHash(value: unknown, name: string): string {
	const hash = requireString(value, name);
	if (!OPERATION_HASH_PATTERN.test(hash)) {
		throw new ProjectMemoryValidationError(
			`${name} must be a canonical sha256 hash.`
		);
	}
	return hash;
}

function requireOperationBody(value: unknown): string {
	if (typeof value !== 'string') {
		throw new ProjectMemoryValidationError(
			'Project-memory operation body must be a string.'
		);
	}
...
function requireGeneration(value: unknown): number {
	if (!Number.isSafeInteger(value) || (value as number) < 0) {
		throw new ProjectMemoryValidationError(
			'Project-memory generation must be a non-negative safe integer.'
		);
	}
...
function requirePageSize(value: unknown): number {
	if (value === undefined) {
		return PROJECT_MEMORY_DEFAULT_PAGE_SIZE;
	}
	if (
		!Number.isSafeInteger(value)
		|| (value as number) < 1
		|| (value as number) > PROJECT_MEMORY_MAX_PAGE_SIZE
	) {
		throw new ProjectMemoryValidationError(
			`Project-memory page_size must be between 1 and ${PROJECT_MEMORY_MAX_PAGE_SIZE}.`
		);
	}
	return value as number;
}
...
function cloneCatalogEntry(
	entry: ProjectMemoryCatalogEntry
): ProjectMemoryCatalogEntry {
	return { ...entry };
}
