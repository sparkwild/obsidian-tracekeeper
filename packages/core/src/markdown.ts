...
interface MaskRange {
	type: Extract<NormalizedVaultSectionType, 'fenced-code' | 'inline-code' | 'html-comment' | 'url'>;
	start: number;
	end: number;
}
...
export function parseMarkdown(rawContent: string): ParsedMarkdown {
	...
	const callouts = extractCalloutBlocks(
		frontmatter.body,
		semantic.masked,
...
function parseFrontmatterBody(frontmatterRaw: string): { fields: Record<string, unknown>; errors: string[] } {
...
function sanitizeYamlRecord(value: Record<string, unknown>): Record<string, unknown> {
...
function sanitizeYamlValue(value: unknown, depth: number): unknown {
	if (depth >= 32) {
		return null;
	}
...
function buildSemanticBody(content: string): SemanticBody {
	...
	ranges.push(
		...findPatternRanges(
			masked,
			/\b[A-Za-z][A-Za-z0-9+.-]*:\/\/[^\s<>"']+|\b(?:mailto|tel|data):[^\s<>"']+/gi,
			'url'
		)
	);
...
function findFencedCodeRanges(content: string): MaskRange[] {
	const ranges: MaskRange[] = [];
...
function maskContent(content: string, ranges: readonly Pick<MaskRange, 'start' | 'end'>[]): string {
	if (ranges.length === 0) {
		return content;
	}
...
function collectHeadings(content: string): ExtractedHeading[] {
...
function extractInternalLinks(content: string, options: LinkExtractionOptions = {}): Wikilink[] {
	...
	const addLink = (
		...
		const position = sourceRangeFromLineOffsets(
			fullText,
			fullTextLineOffsets,
			absoluteStart,
			absoluteEnd
...
function extractFrontmatterLinks(
	...
	const links: NormalizedVaultEdge[] = [];
	let searchOffset = 0;
...
function readMarkdownDestination(value: string): string {
...
function parseLinkTarget(
	value: string
): { linkPath: string; subpath?: string; subpathKind?: 'heading' | 'block' } | null {
...
function buildSections(
	...
	for (const range of semantic.ranges) {
		sections.push({
			...
			position: sourceRange(
				fullText,
				frontmatter.bodyOffset + range.start,
				frontmatter.bodyOffset + range.end
			),
		});
	}
	for (const heading of headings) {
		sections.push({
			...
			position: sourceRange(
				fullText,
				frontmatter.bodyOffset + heading.start,
				frontmatter.bodyOffset + heading.end
			),
		});
	}
...
function sourceRange(fullText: string, start: number, end: number): VaultSourceRange {
	return sourceRangeFromLineOffsets(fullText, lineStartOffsets(fullText), start, end);
}

function sourceRangeFromLineOffsets(
	fullText: string,
...
function sourceLocationFromLineOffsets(
	fullText: string,
	lineOffsets: readonly number[],
	rawOffset: number
): VaultSourceLocation {
...
function releaseRange(start: number, end: number, occupied: Uint8Array): void {
	occupied.fill(0, Math.max(0, start), Math.min(occupied.length, end));
}

function compareEdges(left: NormalizedVaultEdge, right: NormalizedVaultEdge): number {
	return (
		left.position.start.offset - right.position.start.offset ||
		left.position.end.offset - right.position.end.offset ||
...
function collapseMaskedText(value: string): string {
	return value
		.split('\n')
		.map((line) => line.trim())
		.filter(Boolean)
