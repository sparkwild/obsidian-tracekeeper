...
export type MemoryEffectiveState =
	| 'current'
	| 'superseded'
	| 'disputed'
	| 'retracted'
...
export type MemoryLifecycleIssueCode =
	| 'duplicate_memory_id'
	| 'dangling_supersedes'
	| 'dangling_contradicts'
	| 'cross_claim_relation'
	| 'supersession_cycle'
	| 'duplicate_current'
	| 'stale_verification';
...
export interface LegacyMemoryLifecycleRow {
	projection: Exclude<MemoryRecordReadProjection, { kind: 'v2' }>;
...
export interface ResolveMemoryLifecycleInput {
	...
	legacy?: readonly Exclude<MemoryRecordReadProjection, { kind: 'v2' }>[];
	now?: string;
...
export function deriveMemoryGovernance(input: MemoryGovernanceProposal): MemoryGovernanceDecision {
	...
	const authority: MemoryAuthority = input.human_approved
		? 'user'
		: input.source_backed
			? 'source'
			: 'agent';
	let confidence_level: MemoryConfidenceLevel = 'uncertain';
...
export function resolveMemoryLifecycle(input: ResolveMemoryLifecycleInput): MemoryLifecycleProjection {
	...
	const issues: MemoryLifecycleIssue[] = [];
	const byId = new Map<string, MemoryRecord>();
	const duplicateIds = new Set<string>();
	for (const record of ordered) {
		if (byId.has(record.memory_id)) duplicateIds.add(record.memory_id);
		else byId.set(record.memory_id, record);
	}
	...
	for (const record of ordered) {
		if (resolutions.has(record.memory_id)) continue;
	...
	for (const memoryId of duplicateIds) markState(resolutions.get(memoryId), 'review', 'duplicate_memory_id');
	...
	for (const cycle of findSupersessionCycles(supersedes)) {
		...
		for (const memoryId of cycle) markState(resolutions.get(memoryId), 'review', 'supersession_cycle');
	}

	for (const [successorId, targetIds] of supersedes) {
		const successor = resolutions.get(successorId);
		if (!successor || successor.state === 'review' || successor.state === 'retracted') continue;
	...
	for (const group of groups.values()) {
		...
		for (const row of candidates) markState(row, 'disputed', 'duplicate_current');
	}
	...
	const legacy = [...(input.legacy ?? [])]
		.map((projection): LegacyMemoryLifecycleRow => ({
			projection,
			effective_state: 'legacy_unkeyed',
...
function initialState(record: MemoryRecord, now: Date): MemoryEffectiveState {
	if (record.declared_state === 'retracted') return 'retracted';
	if (record.declared_state === 'review') return 'review';
	if (record.declared_state === 'disputed') return 'disputed';
	if (record.valid_from && Date.parse(record.valid_from) > now.getTime()) return 'review';
	if (record.valid_to && Date.parse(record.valid_to) < now.getTime()) return 'superseded';
	return 'current';
}

function initialReasons(record: MemoryRecord, now: Date): string[] {
	if (record.declared_state !== 'active') return [`declared_${record.declared_state}`];
	if (record.valid_from && Date.parse(record.valid_from) > now.getTime()) return ['not_yet_valid'];
	if (record.valid_to && Date.parse(record.valid_to) < now.getTime()) return ['validity_ended'];
	return [];
}

function sameClaim(left: MemoryRecord, right: MemoryRecord): boolean {
	return left.scope === right.scope
		&& left.project_id === right.project_id
		&& left.claim_key === right.claim_key;
}

function groupByClaim(rows: MutableResolution[]): Map<string, MutableResolution[]> {
	const groups = new Map<string, MutableResolution[]>();
...
function claimGroupKey(scope: MemoryScope, projectId: string | null, claimKey: string): string {
	return `${scope}\u0000${projectId ?? ''}\u0000${claimKey}`;
}

function markState(row: MutableResolution | undefined, state: MemoryEffectiveState, reason: string): void {
	if (!row) return;
	const priority: Record<MemoryEffectiveState, number> = {
		current: 0,
		superseded: 1,
		disputed: 2,
		retracted: 3,
	...
	if (priority[state] >= priority[row.state]) row.state = state;
	row.reasons.add(reason);
}

function relationIssue(
	code: 'dangling_supersedes' | 'dangling_contradicts' | 'cross_claim_relation',
	memoryId: string,
	reference: string
): MemoryLifecycleIssue {
	return {
		code,
		memory_ids: [memoryId],
		reference,
...
function findSupersessionCycles(graph: ReadonlyMap<string, readonly string[]>): string[][] {
	...
	for (const node of [...graph.keys()].sort()) if ((state.get(node) ?? 0) === 0) visit(node);
	return [...cycles.values()].sort((left, right) => left.join().localeCompare(right.join()));
}

function freezeResolution(row: MutableResolution): ResolvedMemoryRecord {
	return {
		record: row.record,
...
function compareResolvedRows(left: ResolvedMemoryRecord, right: ResolvedMemoryRecord): number {
...
function dedupeIssues(issues: MemoryLifecycleIssue[]): MemoryLifecycleIssue[] {
	const unique = new Map<string, MemoryLifecycleIssue>();
...
function normalizeNow(value: string | undefined): Date {
	...
	if (Number.isNaN(parsed.getTime())) throw new Error('Memory lifecycle now must be a valid timestamp.');
	return parsed;
}

function normalizeStaleAfterDays(value: number | undefined): number | null {
