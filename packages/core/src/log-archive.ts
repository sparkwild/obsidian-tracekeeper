...
const safeName = (name: string) => !path.isAbsolute(name) && name.split('/').every(part => part !== '..' && part !== '.' && part.length > 0) && !name.includes('\\');
/** 冷分片不可变，按哈希分片索引；调用方必须持有共享写锁。 */
export class LogArchive {
	private readonly root: string;
	constructor(private readonly directory: string) { this.root = path.join(directory, '.cold'); }
	private async key(): Promise<Buffer> {
		const raw = await readLogFile(this.directory, path.join(this.directory, '.payload-encryption-key'), 128);
		if (raw === null)
	...
	private signature(value: Omit<Manifest, 'mac'>, key: Buffer): string { return createHmac('sha256', key).update(JSON.stringify(value)).digest('hex'); }
	private async manifest(): Promise<Manifest | null> {
		const raw = await readLogFile(this.directory, path.join(this.root, 'CURRENT'), 1024 * 1024);
		if (raw === null) {
			const existing = await fs.readdir(this.root).catch(error => {
	...
	private async bucket(manifest: Manifest | null, prefix: string): Promise<Record<string, Entry>> {
		const descriptor = manifest?.buckets[prefix];
		if (!descriptor)
			return {};
		if (!/^[a-f0-9-]+\.index$/.test(descriptor.file))
			throw new Error('Invalid archive index path.');
		const raw = await readLogFile(this.directory, path.join(this.root, descriptor.file), 32 * 1024 * 1024);
		if (raw === null || logHash(raw) !== descriptor.hash)
			throw new Error('Archive index is missing or corrupt.');
		return JSON.parse(raw) as Record<string, Entry>;
	}
	private async readEntry(name: string, entry: Entry): Promise<string> {
		...
		const file = path.join(this.root, entry.segment);
	...
	async read(name: string): Promise<string | null> {
		...
		for (let attempt = 0; attempt < 3; attempt++) {
			...
			try {
				const entry = (await this.bucket(manifest, logHash(name)[0]))[name];
	...
	async entries(): Promise<Map<string, Entry>> {
		const manifest = await this.manifest();
 ...
 async maintenanceState(): Promise<'idle' | 'preparing' | 'publishing' | 'invalid'> {
  const raw=await readLogFile(this.directory,path.join(this.root,'PREPARING'),4*1024*1024);
  if(raw===null)return 'idle';
  try { const parsed=JSON.parse(raw);const {mac,...unsigned}=parsed;const expected=createHmac('sha256',await this.key()).update(JSON.stringify(unsigned)).digest('hex');return mac===expected&&['preparing','publishing'].includes(parsed.phase)?parsed.phase:'invalid'; } catch {return 'invalid';}
 }

	async snapshot(): Promise<LogArchiveSnapshot> {
	...
	async verify(): Promise<LogArchiveSnapshot> {
	...
	async rebuildIndex(): Promise<void> {
		...
		if (!previous)
			return;
		const values = new Map<string, Record<string, Entry>>();
		for (const [segment, expectedHash] of Object.entries(previous.segments)) {
			...
			const raw = await readLogFile(this.directory, path.join(this.root, segment));
			if (raw === null || logHash(raw) !== expectedHash)
			...
			for (const line of raw.split('\n').filter(Boolean)) {
				...
				const prefix = logHash(frame.name)[0], bucket = values.get(prefix) ?? {};
				if (bucket[frame.name])
					throw new Error('Duplicate archive identity.');
				const bytes = Buffer.from(line + '\n');
				bucket[frame.name] = { segment, offset, length: bytes.length, hash: logHash(bytes), originalHash: frame.originalHash };
				offset += bytes.length;
				values.set(prefix, bucket);
			}
		}
		const generation = randomBytes(16).toString('hex'), buckets: Record<string, Bucket> = {};
		for (const [prefix, bucket] of values) {
			const content = JSON.stringify(bucket), file = `${generation}-${prefix}.index`;
			if (Buffer.byteLength(content) > 32 * 1024 * 1024) throw new Error('Archive index capacity reached.');
			await writeLogFile(this.directory, path.join(this.root, file), content);
			buckets[prefix] = { file, hash: logHash(content) };
		}
		const unsigned: Omit<Manifest, 'mac'> = { version: 1, generation, buckets, segments: previous.segments };
		const rebuilt = JSON.stringify({ ...unsigned, mac: this.signature(unsigned, await this.key()) });
		if (Buffer.byteLength(rebuilt) > 1024 * 1024) throw new Error('Archive manifest capacity reached.');
		await writeLogFile(this.directory, path.join(this.root, 'CURRENT'), rebuilt);
		await this.verify();
	}
	async recoverPreparation(): Promise<void> {
		const raw = await readLogFile(this.directory, path.join(this.root, 'PREPARING'), 4 * 1024 * 1024);
		if (raw === null)
			return;
		...
		const expected = createHmac('sha256', await this.key()).update(JSON.stringify(unsigned)).digest('hex');
		...
		const current = await readLogFile(this.directory, path.join(this.root, 'CURRENT'), 1024 * 1024);
		if (intent.phase === 'publishing' && intent.manifest) {
			...
			for (const prefix of Object.keys(planned.buckets))
				for (const [name, entry] of Object.entries(await this.bucket(intent.manifest, prefix)))
					await this.readEntry(name, entry);
			await writeLogFile(this.directory, path.join(this.root, 'CURRENT'), JSON.stringify(intent.manifest));
			await this.retire();
			await fs.unlink(path.join(this.root, 'PREPARING'));
			return;
		}
		...
		for (const original of intent.originals) {
			...
			const bytes = await readLogFile(this.directory, path.join(this.directory, original.name));
			if (bytes === null || logHash(bytes) !== original.hash)
		...
		for (const file of await fs.readdir(this.root))
			if (file.startsWith(intent.generation + '-') && /\.(pack|index)$/.test(file)) {
				await assertLogPath(this.directory, path.join(this.root, file));
				await fs.unlink(path.join(this.root, file));
			}
		await fs.unlink(path.join(this.root, 'PREPARING'));
	}
	async commit(items: LogArchiveItem[]): Promise<void> {
		if (!items.length)
			return;
		await this.recoverPreparation();
		const previous = await this.manifest();
		const generation = randomBytes(16).toString('hex');
		const buckets: Record<string, Bucket> = { ...previous?.buckets };
		const changed = new Map<string, Record<string, Entry>>();
		const segments: Record<string, string> = { ...previous?.segments };
		let frames: Buffer[] = [], segmentBytes = 0, ordinal = 0;
		let segment = `${generation}-${ordinal}.pack`;
		const flush = async () => {
			if (frames.length) {
				const bytes = Buffer.concat(frames);
				await writeLogFile(this.directory, path.join(this.root, segment), bytes);
				segments[segment] = logHash(bytes);
			}
			frames = [];
			segmentBytes = 0;
		};
		...
		const persistIntent = async (intent: object) => {
			const encoded = JSON.stringify({ ...intent, mac: createHmac('sha256', await this.key()).update(JSON.stringify(intent)).digest('hex') });
			if (Buffer.byteLength(encoded) > 4 * 1024 * 1024) throw new Error('Archive preparation exceeds its bounded size.');
			await writeLogFile(this.directory, path.join(this.root, 'PREPARING'), encoded);
		};
		await persistIntent(preparation);
		for (const item of items) {
			if (!safeName(item.name))
				throw new Error('Invalid archive item name.');
			const prefix = logHash(item.name)[0];
			...
			bucket[item.name] = { segment, offset: segmentBytes, length: frame.length, hash: logHash(frame), originalHash: item.originalHash };
			frames.push(frame);
			segmentBytes += frame.length;
		}
		await flush();
		for (const [prefix, values] of changed) {
			for (const item of items.filter(row => logHash(row.name)[0] === prefix)) {
			...
			if (Buffer.byteLength(content) > 32 * 1024 * 1024) throw new Error('Archive index capacity reached; hot records retained.');
			await writeLogFile(this.directory, path.join(this.root, file), content);
			buckets[prefix] = { file, hash: logHash(content) };
		}
		const unsigned: Omit<Manifest, 'mac'> = { version: 1, generation, buckets, segments };
		const manifest = { ...unsigned, mac: this.signature(unsigned, await this.key()) };
		if (Buffer.byteLength(JSON.stringify(manifest)) > 1024 * 1024) throw new Error('Archive manifest capacity reached; hot records retained.');
		await persistIntent({ ...preparation, phase: 'publishing', manifest });
		await writeLogFile(this.directory, path.join(this.root, 'CURRENT'), JSON.stringify(manifest));
		await this.retire();
		await fs.unlink(path.join(this.root, 'PREPARING')).catch(error => {
			if (!isMissingLogFile(error))
				throw error;
		});
	}
	...
	async retire(): Promise<void> {
		const hotNames = new Set(await fs.readdir(this.directory));
		for (const [name, entry] of await this.entries()) {
			if (!hotNames.has(name.split('/')[0]))
				continue;
			const file = path.join(this.directory, name), content = await readLogFile(this.directory, file);
			if (content === null)
				continue;
			if (logHash(content) !== entry.originalHash)
